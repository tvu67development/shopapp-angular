  ...
  ngOnInit(): void {  
    ...
    this.userService.getUserDetail(this.token).subscribe({
      next: (response: any) => {
        ...
        this.userResponse = {
          ...response,
          date_of_birth: new Date(response.date_of_birth + 25200000), 
          // nếu ko cộng thêm số này, khi chuyển đổi qua thì chỉ được 1996-10-25 (lùi 1 ngày)
        };    
        this.userProfileForm.patchValue({
          fullname: this.userResponse?.fullname ?? '',
          address: this.userResponse?.address ?? '',
          date_of_birth: this.userResponse?.date_of_birth.toISOString().split('T')[0],
        });        
